...
public class ResultadoDaoImpl implements ResultadoDao {
	...
	public void actualizarResultado(Resultado resultado) {
		...
		if (resultado.getDia() != null) {
			dateSQL = new java.sql.Date(resultado.getDia().getTime());
		}

		String sql = "UPDATE RESULTADOS SET RIVAL = ?, RESULTADO = ?, DIA = ?, HORA = ?, CAMPO = ? WHERE CATEGORIA = ? AND EQUIPO = ?";
		jdbcTemplate.update(sql, resultado.getRival(), resultado.getResultado(), dateSQL, resultado.getHora(),
				resultado.getCampo(), resultado.getCategoria(), resultado.getEquipo());
	}
